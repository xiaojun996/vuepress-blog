...
给你一个按升序排序的整数数组 <span style="background: #eee; color: #666;">num</span>（可能包含重复数字），请你将它们分割成一个或多个子序列，其中每个子序列都由连续整数组成且长度至少为 3 。

如果可以完成上述分割，则返回 <span style="background: #eee; color: #666;">true</span> ；否则，返回 <span style="background: #eee; color: #666;">false</span> 。
...
- 输入的数组长度范围为<span style="background: #eee; color: #666;">[1, 10000]</span>

### 贪心法 ➕ 双 Hash

- 用两个哈希表
  - 第一个 <span style="background: #eee; color: #666;">MapHash</span> 存数字剩余的次数
  - 第一个 <span style="background: #eee; color: #666;">ArrCountHash</span> 存以该数字结尾的 子序列个数
- 先遍历生成 <span style="background: #eee; color: #666;">MapHash</span> 剩余的次数
- 再遍历一遍，遍历时候每次都会从 <span style="background: #eee; color: #666;">MapHash</span> 取出一个元素
  - 一开始 <span style="background: #eee; color: #666;">ArrCountHash</span> 是空的
  - 所以要查找**后面两个**元素，抽出来，凑出一个**合法的序列**（依据题意，长度为 3）
    - 抽出来 <span style="background: #eee; color: #666;">MapHash(num + 1)</span> 和 <span style="background: #eee; color: #666;">MapHash(num + 2)</span>就都要 减 1
    - 如果 <span style="background: #eee; color: #666;">ArrCountHash</span> 有了，就 ➕ 1，没有就为 1
  - 然后 <span style="background: #eee; color: #666;">ArrCountHash</span>就不是空的了，如果存在以 num - 1 结尾的连续子序列，则将 num 放入
