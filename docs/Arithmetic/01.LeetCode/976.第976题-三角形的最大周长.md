...
给定由一些正数（代表长度）组成的数组 <span style="background: #eee; color: #666;">A</span>，返回由其中三个长度组成的、**面积不为零**的三角形的最大周长。

如果不能形成任何面积不为零的三角形，返回 <span style="background: #eee; color: #666;">0</span>。
