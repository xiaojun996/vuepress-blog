...
给定一个范围在   <span style="background: #ddd; color: #666; padding: 3px 5px; border-radius: 2px;">1 ≤ a[i] ≤ n</span> ( n = 数组大小 ) 的 整型数组，数组中的元素一些出现了两次，另一些只出现一次。

找到所有在 <span style="background: #ddd; color: #666; padding: 3px 5px; border-radius: 2px;">[1, n]</span> 范围之间没有出现在数组中的数字。

您能在不使用额外空间且时间复杂度为 <span style="background: #ddd; color: #666; padding: 3px 5px; border-radius: 2px;">O(n)</span> 的情况下完成这个任务吗? 你可以假定返回的数组不算在额外空间内
